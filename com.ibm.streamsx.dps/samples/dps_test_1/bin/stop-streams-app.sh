...
streamtool lsjobs -i $streams_instance_name | grep -i Running > "$temp_my_streams_jobs_info_file"
# Get the total number of jobs running under this instance.
total_jobs_cnt=$(streamtool lsjobs -i $streams_instance_name | grep -i Running | wc -l)
...
do
   ...
   streamtool canceljob -i $streams_instance_name $job_id
