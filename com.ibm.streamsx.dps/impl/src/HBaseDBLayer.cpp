...
namespace distributed
{
  ...
  void HBaseDBLayer::releaseGeneralPurposeLock(string const & entityName) {
   ...
   void HBaseDBLayer::getKeys(uint64_t store, std::vector<unsigned char *> & keysBuffer, std::vector<uint32_t> & keysSize, int32_t keyStartPosition, int32_t numberOfKeysNeeded, PersistenceError & dbError) {
    ...
    // This method will get the values for a given list of keya from the given store without
    // performing any checks for the existence of store, key etc. This is a 
    // faster version of the get method above. 
    void HBaseDBLayer::getValues(uint64_t store, std::vector<char *> const & keyData, std::vector<uint32_t> const & keySize, std::vector<unsigned char *> & valueData, std::vector<uint32_t> & valueSize, PersistenceError & dbError) {
       SPLAPPTRC(L_DEBUG, "Inside getValues for store id " << store, "HBaseDBLayer");
    ...
    void HBaseDBLayer::putKVPairs(uint64_t store, std::vector<char *> const & keyData, std::vector<uint32_t> const & keySize, std::vector<unsigned char *> const & valueData, std::vector<uint32_t> const & valueSize, PersistenceError & dbError) {
       SPLAPPTRC(L_DEBUG, "Inside putKVPairs for store id " << store, "HBaseDBLayer");
    ...
    void HBaseDBLayer::hasKeys(uint64_t store, std::vector<char *> const & keyData, std::vector<uint32_t> const & keySize, std::vector<bool> & results, PersistenceError & dbError) {
       SPLAPPTRC(L_DEBUG, "Inside hasKeys for store id " << store, "HBaseDBLayer");
    ...
    void HBaseDBLayer::removeKeys(uint64_t store, std::vector<char *> const & keyData, std::vector<uint32_t> const & keySize, int32_t & totalKeysRemoved, PersistenceError & dbError) {
       SPLAPPTRC(L_DEBUG, "Inside removeKeys for store id " << store, "HBaseDBLayer");
