...
namespace distributed
{
  ...
  void CassandraDBLayer::releaseGeneralPurposeLock(string const & entityName) {
   ...
   void CassandraDBLayer::getKeys(uint64_t store, std::vector<unsigned char *> & keysBuffer, std::vector<uint32_t> & keysSize, int32_t keyStartPosition, int32_t numberOfKeysNeeded, PersistenceError & dbError) {
    ...
    void CassandraDBLayer::getValue(std::string const & storeIdString, char const * & key, uint32_t const & keySize, unsigned char * & value, uint32_t & valueSize, uint64_t & error) {
       SPLAPPTRC(L_DEBUG, "Inside getValue for store id " << storeIdString, "CassandraDBLayer");
